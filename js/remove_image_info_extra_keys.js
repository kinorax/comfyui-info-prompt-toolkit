...
const TARGET_NODE_TYPES = new Set([
    "IPT-RemoveImageInfoExtraKeysV2",
    "RemoveImageInfoExtraKeysV2",
    "Remove Image Info Extra Keys",
]);
const BACKING_WIDGET_NAME = "items";
const EDITOR_WIDGET_NAME = "IPT-extra-key-toggle-list-editor";
const EDITOR_WIDGET_TYPE = "IPT-ExtraKeyToggleListEditor";
const STYLE_ID = "ipt-remove-image-info-extra-keys-style";
const STATE_KEY = "__iptRemoveImageInfoExtraKeysEditorState";
const ROW_HEIGHT = 30;
...
function installStyleOnce() {
    ...
    style.textContent = `
        ...
        .ipt-extra-key-list__rows {
            display: flex;
            flex-direction: column;
            gap: 4px;
            overflow: visible;
        }
        .ipt-extra-key-list__row {
            min-width: 0;
        ...
        .ipt-extra-key-list__input:focus {
            border-color: var(--p-primary-color, #6b9cff);
        }
        .ipt-extra-key-list__input::placeholder {
            color: var(--component-node-foreground-secondary, #999);
        }
        .ipt-extra-key-list__switch {
            position: relative;
            width: 32px;
            height: 18px;
            padding: 0;
        ...
        .ipt-extra-key-list__switch::after {
            content: "";
            position: absolute;
            top: 2px;
            left: 2px;
        ...
        .ipt-extra-key-list__switch[aria-checked="true"] {
            background: var(--p-primary-color, #4778d7);
            border-color: var(--p-primary-color, #4778d7);
        }
        .ipt-extra-key-list__switch[aria-checked="true"]::after {
            transform: translateX(14px);
            background: #fff;
        }
        .ipt-extra-key-list__switch:focus-visible,
        .ipt-extra-key-list__delete:focus-visible {
            outline: 2px solid var(--p-primary-color, #6b9cff);
            outline-offset: 1px;
        }
        .ipt-extra-key-list__switch.is-placeholder,
        .ipt-extra-key-list__delete.is-placeholder {
            visibility: hidden;
            pointer-events: none;
        }
        .ipt-extra-key-list__delete {
            width: 24px;
            height: 24px;
            padding: 0;
            color: var(--component-node-foreground-secondary, #aaa);
...
function createEditor(node, backingWidget) {
    ...
    host.className = "ipt-extra-key-list";

    const header = document.createElement("div");
    header.className = "ipt-extra-key-list__header";
    header.textContent = "remove";

    const rowsElement = document.createElement("div");
    rowsElement.className = "ipt-extra-key-list__rows";
    ...
    const render = (focusRowId = null) => {
        ...
        for (let index = 0; index < rows.length; index += 1) {
            ...
            rowElement.className = "ipt-extra-key-list__row";

            const toggle = document.createElement("button");
            toggle.type = "button";
            toggle.className = "ipt-extra-key-list__switch";
            toggle.setAttribute("role", "switch");
            toggle.setAttribute("aria-label", `Enable removal for ${row.key || "key"}`);
            ...
            input.className = "ipt-extra-key-list__input";
            input.type = "text";
            input.value = row.key;
            input.placeholder = isPlaceholder ? "key to remove..." : "";
            input.dataset.rowId = String(row.id);
            input.setAttribute("aria-label", "Extra key to remove");
            ...
            remove.type = "button";
            remove.className = "ipt-extra-key-list__delete";
            remove.textContent = "×";
    ...
    editorWidget = node.addDOMWidget(
        ...
        host,
        {
            ...
            onDraw: (widget) => {
                // Undo the frontend's leaked Vue wrapper width before Classic lays out the DOM overlay.
...
function installEditor(node) {
    ...
    if (!state) {
        try {
            ...
            // Keep the standard STRING widget visible as the progressive fallback.
            console.error("[IPT] Failed to create Remove Image Info Extra Keys editor.", error);
            return;
        }
...
app.registerExtension({
    name: "IPT.RemoveImageInfoExtraKeysEditor",
