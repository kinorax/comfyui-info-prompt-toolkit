...
def build_sam3_image_model(
    ...
    """
    if not eval_mode:
        raise RuntimeError("Vendored SAM3 image build only supports eval_mode=True")
    if enable_inst_interactivity:
        raise RuntimeError(
            "Vendored SAM3 image build does not support instance interactivity"
        )
...
def download_ckpt_from_hf():
    try:
        ...
        raise RuntimeError(
            "huggingface_hub is required only when loading SAM3 checkpoints from Hugging Face"
        ) from exc
